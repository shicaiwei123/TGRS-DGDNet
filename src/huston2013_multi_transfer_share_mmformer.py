...
from loss.kd import *
from lib.model_develop import train_knowledge_distill_fc_feature_share
...
def deeppix_main(args):
    ...
    args.p = [1, 1]
    teacher_model = HSI_Lidar_Couple_Former(args, modality_1_channel, modality_2_channel)

    args.p = [0, 0]
    student_model = HSI_Lidar_Couple_Former(args, modality_1_channel, modality_2_channel)
    ...
    if 'lidar' in args.pair_modalities:
        ...
        teacher_model.load_state_dict(
            torch.load(os.path.join(args.model_root, 'mmformer_full/'+"huston2013_"+args.pair_modalities[0]+'_'+args.pair_modalities[1]+'_couple_cross_fc_version_' + str(1) + '_unimodal_center_2.5.pth')))
        teacher_model.eval()
    elif 'ms'  in args.pair_modalities:
        teacher_model.load_state_dict(
            torch.load(os.path.join(args.model_root, 'share_unimodal_center_full/hsi_ms_couple_cross_fc_version_2_labma_unimodal0.5.pth')))
        teacher_model.eval()
    else:
    ...
    args.model_root = os.path.join(args.model_root,'share_center_1.5_transfer_mv', 'kd_'+args.location+'pooling', 'margin_' + str(args.margin))
    args.log_root = os.path.join(args.log_root, 'share_center_1.5_transfer_mv','kd_'+args.location+'pooling', 'margin_' + str(args.margin))
    ...
    train_knowledge_distill_fc_feature_share(net_dict=nets, cost_dict=criterions, optimizer=optimizer,
