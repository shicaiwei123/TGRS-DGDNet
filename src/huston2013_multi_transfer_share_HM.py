...
def deeppix_main(args):
    ...
    teacher_model.load_state_dict(
        torch.load(os.path.join(args.model_root, 'share_drop/hsi_ms_couple_cross_fc_version_' + str(2) + '.pth')))
    teacher_model.eval()
    for param in teacher_model.parameters():
        param.requires_grad = False

    args.model_root = os.path.join(args.model_root,'share_transfer', 'kd', 'margin_' + str(args.margin))
    args.log_root = os.path.join(args.log_root, 'share_transfer','kd', 'margin_' + str(args.margin))
