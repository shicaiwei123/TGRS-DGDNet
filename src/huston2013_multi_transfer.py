...
def deeppix_main(args):
    ...
    args.p = [1, 1]
    teacher_model = HSI_Lidar_Couple_Cross_Drop(args, modality_1_channel, modality_2_channel)

    args.p = [0, 0]
    student_model = HSI_Lidar_Couple_Cross_Drop(args, modality_1_channel, modality_2_channel)


    # 初始化并且固定teacher 网络参数
    teacher_model.load_state_dict(
        torch.load(os.path.join(args.model_root, 'drop/hsi_lidar_couple_cross_fc_version_2.pth')))
    teacher_model.eval()
    for param in teacher_model.parameters():
        param.requires_grad = False

    args.model_root = os.path.join(args.model_root, 'kd', 'margin_' + str(args.margin))
    args.log_root = os.path.join(args.log_root, 'kd', 'margin_' + str(args.margin))
