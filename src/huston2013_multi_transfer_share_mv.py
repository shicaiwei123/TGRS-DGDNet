...
def deeppix_main(args):
    ...
    args.p = [1, 1]
    teacher_model = SURF_MV(args, modality_1_channel, modality_2_channel)

    args.p = [0, 0]
    student_model = SURF_MV(args, modality_1_channel, modality_2_channel)
    ...
    if 'lidar' in args.pair_modalities:
        ...
        teacher_model.load_state_dict(
            torch.load(os.path.join(args.model_root, 'mv_full/'+args.pair_modalities[0]+'_'+args.pair_modalities[1]+'_7x7_couple_cross_fc_version_' + str(1) + '_labma_unimodal3.0.pth')))
        teacher_model.eval()
