...
def deeppix_main(args):
    ...
    if 'lidar' in args.pair_modalities:
        ...
        teacher_model.load_state_dict(
            torch.load(os.path.join(args.model_root,
                                    'share_unimodal_center_full/hsi_lidar_' + str(args.patch_size) + 'x' + str(
                                        args.patch_size) + '_couple_cross_fc_version_1_labma_unimodal3.0' + '.pth')))
        teacher_model.eval()
    elif 'ms' in args.pair_modalities:
        teacher_model.load_state_dict(
            torch.load(os.path.join(args.model_root,
                                    'share_unimodal_center_full/hsi_ms_couple_cross_fc_version_2_labma_unimodal0.5.pth')))
        teacher_model.eval()
    else:
    ...
    args.model_root = os.path.join(args.model_root, 'share_center_transfer' + str(args.patch_size) + 'x' + str(
        args.patch_size), 'kd_' + args.location + 'pooling',
                                   'margin_' + str(args.margin))
    args.log_root = os.path.join(args.log_root, 'share_center_transfer' + str(args.patch_size) + 'x' + str(
        args.patch_size), 'kd_' + args.location + 'pooling',
                                 'margin_' + str(args.margin))
