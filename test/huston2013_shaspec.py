...
def deeppix_main_share(args):
    ...
    for i in range(3):
        ...
        model.load_state_dict(
            torch.load(
                os.path.join(args.model_root, 'shaspec_drop/' + "hsi_ms_7x7_couple_cross_fc_version_" + str(
                    i+3) + "_labma_unimodal1.0.pth")))
        model.eval()
...
def augsburg_main_share(args):
    ...
    for i in range(3):
        ...
        path = os.path.join(args.model_root,
                            'share_center_1.5_transfer/kd_afterpooling/margin_0/dad_hsi+ms_lr_0.001_version_' + str(
                                i+1) + '_lambda_kd_feature_0.2_intra_0.pth')
        ...
        model.load_state_dict(
            torch.load(
                os.path.join(args.model_root,
                             'mmformer_drop/augsburg_hsi_sar_dsm_couple_cross_fc_version_' + str(i) + '.pth')))
        model.eval()
