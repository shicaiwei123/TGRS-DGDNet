...
sys.path.append('..')
from models.surf_corr_repr import SURF_MV,SURF_MV_TRI
...
def deeppix_main_share(args):
    ...
    for i in range(3):
        ...
        model.load_state_dict(
            torch.load(
                os.path.join(args.model_root, 'mv_full/'+args.pair_modalities[0]+'_'+args.pair_modalities[1]+'_7x7_couple_cross_fc_version_' + str(i) + '_labma_unimodal2.0.pth')))
        model.eval()
        ...
        model.load_state_dict(
            torch.load(
                os.path.join(args.model_root,
                             'mv_drop/augsburg_' + 'hsi_sar_dsm' + '_couple_cross_fc_version_' + str(i) + '.pth')))
        model.eval()
